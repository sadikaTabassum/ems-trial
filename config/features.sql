...
CREATE OR REPLACE PROCEDURE CREATE_HOTEL (
    ...
    $$ DECLARE HOTEL_ALREADY_EXIST EXCEPTION;
BEGIN
    IF EXISTS (
        SELECT
            1
...
EXCEPTION
    WHEN HOTEL_ALREADY_EXIST THEN
        RAISE NOTICE 'Hotel already exists!...';
    WHEN OTHERS THEN
...
CREATE OR REPLACE PROCEDURE CREATE_HOTEL_ROOM ( P_H_ID INT, T_S_ROOM INT, T_M_ROOM INT, T_L_ROOM INT, A_S_ROOM INT, A_M_ROOM INT, A_L_ROOM INT ) AS
